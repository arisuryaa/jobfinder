...
$res = $stmt->get_result();
?>
<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Dashboard Pelamar</title>
...
<?php while($r = $res->fetch_assoc()): ?>
  <div class="d-flex justify-content-between align-items-center mb-2">
    <div><strong><?=htmlspecialchars($r['title'])?></strong><div class="small text-muted"><?=htmlspecialchars($r['applied_at'])?></div></div>
    <div><a class="btn btn-sm btn-outline-secondary" href="../uploads/<?=urlencode($r['cv_file'])?>" target="_blank">Download CV</a></div>
  </div>
<?php endwhile; ?>
