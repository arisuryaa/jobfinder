    ...
    <div class="container py-4"><a href="index.php" class="btn btn-link">&larr; Kembali</a>
        <div class="card">
            <div class="card-body">
                ...
                <p class="text-muted"><?=htmlspecialchars($job['company'])?> — <?=htmlspecialchars($job['location'])?> ·
                    <small><?=htmlspecialchars($job['category'])?></small></p>
                <p><strong>Type:</strong> <?=htmlspecialchars($job['type'])?> | <strong>Salary:</strong>
                    <?=htmlspecialchars($job['salary'])?></p>
